...
namespace geston_vente
{
    ...
    internal class DbConnexion
    {
        ...
        {
            string req1 = "INSERT INTO contrats (ref_ter, ref_cli, ref_ven, conditions, nb_paie, deb, fin, penalite) VALUES (@RefTer, @RefCli, @RefVen, @Conditions, @NbPaie, @Deb, @Fin, @Penalite)";
        ...
        {
            string query = "UPDATE contrats SET ref_ter=@RefTer, ref_cli= @RefCli, ref_ven= @RefVen, conditions= @Conditions, nb_paie=@NbPaie, deb=@Deb, fin=@Fin, penalite=@Penalite WHERE num_cont = @NumCont WHERE num_cont = @NumCont";
            MySqlConnection connexion = DBconnexion();
